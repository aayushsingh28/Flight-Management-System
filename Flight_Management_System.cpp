...
const vector<string> l_destination = {"LUCKNOW", "DELHI", "CHANDIGARH", "BANGALORE", "CHENNAI"};														   // local Destination
...
public:
	...
	back:
		cout << endl
			 << endl
			 << "Enter the type of Complaint you want to register:\n1.Airport Handling\n2.Baggage Delivery\n3.Booking and ticketing\n4.Other\n";
		choice = getch();
		cin.ignore();
		...
		cout << endl
			 << "Do you want to edit your Complaint(y/n): ";
		ch = getch();
		if (ch == 'Y' || ch == 'y')
			goto back;
		else
...
phone1:
	...
	if (phone_no.size() < 10)
	{
		cout << "Enter correct phone number." << endl;
		goto phone1;
	}
...
int main()
{
	cout << "\n\t\t\t\t\tOOPS INNOVATIVE PROJECT\n\n\t\t\t\t\tFLIGHT MANAGEMENT SYSTEM\n"
		 << endl;
	cout << "\t\t\t\t\t\tMADE BY\n\t\t\t\t\tAAYUSH SINGH : 2K20/IT/03\n\t\t\t\t\tAMAN KUMAR MEENA : 2K20/IT/17" << endl;
	cout << endl
...
back2menu:
	...
	back:
		...
		else if (choice == 50)
		{
			...
			cout << endl
				 << "Enter Your Passport Number(Without \"-\"): ";
			cin >> pass_no;
			b = ptr->idest(pass_no);
		}
		else if (choice == 51)
